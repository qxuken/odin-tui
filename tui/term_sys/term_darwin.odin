...
@(private = "file")
syscall_ioctl :: #force_inline proc "contextless" (fd: c.int, request: u32, arg: uintptr) -> c.int {
    return cast(c.int)intrinsics.syscall(darwin.unix_offset_syscall(.ioctl), uintptr(fd), uintptr(request), arg)
}
...
_get_size :: proc() -> Window_Size {
    ...
    ensure(ret == 0, "syscall_ioctl return code non zero")
